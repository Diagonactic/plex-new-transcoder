...
static videotoolbox_set_frame(AVCodecContext *avctx, VTContext *vtctx,
                              AVFrame *frame)
{
...
static int videotoolbox_finish_frame(AVCodecContext *avctx, AVFrame *frame)
{
    if (!frame->buf[0] || frame->data[3]) {
        av_log(avctx, AV_LOG_ERROR, "videotoolbox: invalid state when returning frame\n");
        av_frame_unref(frame);
        return AVERROR_EXTERNAL;
    }
    ...
    if (!ref) {
        av_log(NULL, AV_LOG_ERROR, "videotoolbox: no frame decoded?\n");
        av_frame_unref(frame);
        return AVERROR_EXTERNAL;
    }
...
static int videotoolbox_common_end_frame(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    if (!vtctx->frame) {
        av_log(avctx, AV_LOG_ERROR, "videotoolbox: no frame\n");
        return AVERROR_UNKNOWN;
    }

    return videotoolbox_set_frame(avctx, vtctx, frame);
}
...
AVHWAccel ff_h263_videotoolbox_hwaccel = {
    ...
    .finish_frame   = videotoolbox_finish_frame,
