...
static int asf_write_header1(AVFormatContext *s, int64_t file_size, int64_t data_chunk_size)
{
    ...
    avio_wl32(pb, 3 + has_title + !!metadata_count + s->nb_streams + !!seek_offset); /* number of chunks in header */
    avio_w8(pb, 1); /* ??? */
    avio_w8(pb, 2); /* ??? */
...
static int asf_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    put_frame(s, stream, s->streams[pkt->stream_index], pkt->dts + asf->offset, pkt->data, pkt->size, flags);
