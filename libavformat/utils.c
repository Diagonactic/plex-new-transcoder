    ...
    for(;;){
        ...
        if(st->request_probe>0){
            ...
            if(end || av_log2(pd->buf_size) != av_log2(pd->buf_size - pkt->size)){
				//PLEX
				int score= set_codec_from_probe_data(s, st, pd); //, 1/*st->probe_packets > 0 ? AVPROBE_SCORE_MAX/4 : 0*/);
...
//                    av_log(NULL, AV_LOG_ERROR, "%f\n", dur);
				...
				if (st->info->max_duration == 0)
				{
					st->info->min_duration = duration;
					st->info->max_duration = duration;
				}
				else
				{
					if (st->info->min_duration > duration) st->info->min_duration = duration;
					if (st->info->max_duration < duration) st->info->max_duration = duration;
				}
            }
...
static void dump_stream_format(AVFormatContext *ic, int i, int index, int is_output)
{
    ...
    snprintf(stream, 1024, "    Stream #%d.%d", index, i);
    ...
    if (flags & AVFMT_SHOW_IDS)
    {
        ...
        av_strlcatf(stream, 1024, "[0x%x]", st->id);
    }
      
    if (lang)
    {
        av_log(NULL, AV_LOG_INFO, "(%s)", lang->value);
        av_strlcatf(stream, 1024, "(%s)", lang->value);
    }
      
    av_log(NULL, AV_LOG_DEBUG, ", %d, %d/%d", st->codec_info_nb_frames, st->time_base.num/g, st->time_base.den/g);
    av_strlcatf(stream, 1024, ", %d, %d/%d", st->codec_info_nb_frames, st->time_base.num/g, st->time_base.den/g);
    
    av_log(NULL, AV_LOG_INFO, ": %s", buf);
    av_strlcatf(stream, 1024, ": %s", buf);
    ...
    if (st->sample_aspect_ratio.num && // default
        ...
        av_strlcatf(stream, 1024, ", SAR %d:%d DAR %d:%d",
                        st->sample_aspect_ratio.num, st->sample_aspect_ratio.den,
                        display_aspect_ratio.num, display_aspect_ratio.den);
    }
    if(st->codec->codec_type == AVMEDIA_TYPE_VIDEO){
    ...
    av_log(NULL, AV_LOG_INFO, " (codecdata: %d bytes)", st->codec->extradata_size);

    av_log(NULL, AV_LOG_INFO, "\n");
    dump_metadata(NULL, st->metadata, "    ");
    
    PMS_Log(LOG_LEVEL_DEBUG, stream);
}
                    ...
                    int is_output)
{
    ...
    PMS_Log(LOG_LEVEL_DEBUG, "%s #%d, %s, %s '%s':\n",
                  is_output ? "Output" : "Input",
                  index,
                  is_output ? ic->oformat->name : ic->iformat->name,
                  is_output ? "to" : "from", url);
