...
#endif
static int64_t plex_start_time = 0;
static int64_t plex_seek_offset = 0;
...
static int configure_video_filters(InputStream *ist, OutputStream *ost)
{
    ...
    double stream_width = icodec->width;
    double stream_height = icodec->height;
    ...
    // Snap both dimensions to multiple of 2 in case client asked for something odd.
                         ...
                         const AVPacket *pkt)
{
    ...
    handle_eof:
        ...
        if (rate_emu) {
            ...
            now += (start_time + rate_emu_buf) * AV_TIME_BASE;
            if (pts > now)
                usleep(pts - now);
        }
                     ...
                     StreamMap *stream_maps, int nb_stream_maps)
{
    ...
    // PLEX.
    {
        AVFormatContext* ctx = input_files[0].ctx;
        char str[128];

        // Duration of the part we're transcoding.
        sprintf(str, "%g", (ctx->duration-plex_start_time) / (double)AV_TIME_BASE);
        av_dict_set(&ctx->metadata, "plex.duration", str, 0);

        // Duration of the entire thing.
        sprintf(str, "%g", (ctx->duration) / (double)AV_TIME_BASE);
        av_dict_set(&ctx->metadata, "plex.total_duration", str, 0);

        if (plex_start_time != 0)
        {
            sprintf(str, "%lld", plex_start_time);
            av_dict_set(&ctx->metadata, "plex.start_offset", str, 0);
        }

        if (plex_seek_offset != 0)
        {
          sprintf(str, "%lld", plex_seek_offset);
          av_dict_set(&ctx->metadata, "plex.seek_offset", str, 0);
        }
    }
...
{
    plex_start_time = start_time = parse_time_or_die(opt, arg, 1);
    return 0;
}
...
static void new_video_stream(AVFormatContext *oc, int file_idx)
{
    ...
    if (video_stream_copy) {
        ...
        AVFormatContext* ctx = input_files[0].ctx;
        if (ctx && ctx->streams && ctx->streams[0]->codec->codec_type == AVMEDIA_TYPE_VIDEO)
        {
          AVStream* inputStream = ctx->streams[0];

          // See if things are sane.
          long int averageFPS = 0;
          if (inputStream->avg_frame_rate.den != 0)
            averageFPS = (int)(av_q2d(inputStream->avg_frame_rate)+0.5);

          long int framerateFPS = 0;
          if (inputStream->r_frame_rate.den != 0)
            framerateFPS = (int)(av_q2d(inputStream->r_frame_rate)+0.5);

          PMS_Log(3, "Average FPS ~ %d fps, Frame rate ~ %d fps.", averageFPS, framerateFPS);

          // Don't trust any super high framerates.    
          if (framerateFPS > 100 && inputStream->avg_frame_rate.den != 0)
          {
              PMS_Log(3, "Codec frame rate seems really high, we'll attempt to fix.");
              PMS_Log(3, "Forcing fps to frame rate of %f.\n", av_q2d(inputStream->avg_frame_rate));
              ost->frame_rate = inputStream->avg_frame_rate;
          }
        }
...
int plex_opt_cookie(const char *opt, const char *arg);
int plex_opt_user_agent(const char *opt, const char *arg);

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int plex_opt_cookie(const char *opt, const char *arg)
{
    HTTP_COOKIES = av_strdup(arg);
    return 0;
}
...
#include "cmdutils_common_opts.h"
    ...
    { "seek_offset", HAS_ARG | OPT_INT64 | OPT_EXPERT, {(void*)&plex_seek_offset}, "offset used to align packets when seeking", "number" },
    ...
    { "rebuf", HAS_ARG | OPT_INT | OPT_EXPERT, {(void*)&rate_emu_buf}, "allowed number of seconds to be ahead when reading input at native frame rate", "seconds" },
    ...
    { "cookie", HAS_ARG | OPT_EXPERT, {(void*)plex_opt_cookie}, "set the cookie we need to pas along with our http requests", "" },
    { "useragent", HAS_ARG | OPT_EXPERT, {(void*)plex_opt_user_agent}, "set the user agent we need to pas along with our http requests", "" },
    
    { NULL, },
};

extern int main_plex(int argc, char **argv);
int main(int argc, char **argv)
{
 ...
 // See if we're doing a stream to me transcode or now.
    if (argc > 1 && (strcmp(argv[1], "transcode") == 0 || strcmp(argv[1], "fontcache") == 0))
    {
      // Vector off and exit.
      return main_plex(argc, argv);
    } 
